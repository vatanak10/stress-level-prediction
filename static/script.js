...
function createEmojiRating(fieldName, min = 0, max = 5) {
  const emojis = [
    // Rating 1 - Very sad
  ...
  let html = '<ul class="flex gap-2 justify-center">';

  for (let i = 1; i <= max; i++) {
    html += `
      <li>
        <svg
          class="w-10 text-gray-300 hover:scale-105 cursor-pointer transition-all"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 496 512"
          data-rating="${i}"
          onclick="setRating('${fieldName}', ${i})"
        >
          <path fill="currentColor" d="${emojis[i - 1]}"></path>
        </svg>
      </li>
    `;
  }
...
function setRating(fieldName, rating) {
  ...
  document.getElementById(fieldName + "_value").textContent = rating;

  // Update emoji colors
  const parent = document
    .querySelector(`svg[onclick*="${fieldName}"]`)
    .closest("ul");
  const emojiList = parent.querySelectorAll("svg");

  emojiList.forEach((emoji) => {
    const emojiRating = parseInt(emoji.getAttribute("data-rating"));
    if (emojiRating <= rating) {
      emoji.classList.remove("text-gray-300");
      emoji.classList.add("text-[#ac4dbd]");
    } else {
      emoji.classList.add("text-gray-300");
      emoji.classList.remove("text-[#ac4dbd]");
    }
  });
}

// Initialize emoji ratings when DOM is loaded
document.addEventListener("DOMContentLoaded", function () {
  const emojiContainers = document.querySelectorAll("[data-emoji-rating]");
  emojiContainers.forEach((container) => {
    const fieldName = container.getAttribute("data-emoji-rating");
    const max = parseInt(container.getAttribute("data-max")) || 5;
    container.innerHTML = createEmojiRating(fieldName, 0, max);
  });
});
